...
const normalizeMarkdown = (text: string) => {
    ...
    lines.forEach((line) => {
        ...
        if (bullet) {
            ...
            const indent =
                pendingOrdered && leading.length === 0 ? "    " : leading;
...
export const ChatPanel = ({
    ...
    useEffect(() => {
        ...
        const isInside = (node: globalThis.Node | null) =>
            node ? container.contains(node) : false;
    ...
    const messageKey = (m: ChatMessageView) =>
        `${m.nodeId}-${m.createdAt}-${m.content.length}`;
                    ...
                    {history.map((m, idx) => (
                        <div
                            ...
                            className={`bubble-flat ${m.role} ${m.pending ? "pending" : ""} ${
                                streaming && streaming.key === messageKey(m)
                                    ? "streaming"
                                    : ""
                            }`}
                            data-chat-item
