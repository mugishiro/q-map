...
describe("ChatPanel", () => {
    ...
    it("Ctrl+Enter で送信する", async () => {
        ...
        await waitFor(() =>
            expect(onSend).toHaveBeenCalledWith({ message: "質問" }),
        );
    ...
    it("選択したテキストに対して操作ボタンを表示する", async () => {
        ...
        const node = baseNode({
            messages: [
                { role: "user", content: "選択テキスト", createdAt: now },
            ],
        });
        ...
        try {
            ...
            expect(
                await screen.findByRole("group", { name: "選択テキスト操作" }),
            ).toBeInTheDocument();
            expect(
                screen.getByRole("button", { name: "聞く" }),
            ).toBeInTheDocument();
            expect(
                screen.getByRole("button", { name: "あとで聞く" }),
            ).toBeInTheDocument();
    ...
    it("chat-log-body 内にメッセージを描画する", () => {
        ...
        const body = container.querySelector(".chat-log-body");
        expect(log).not.toBeNull();
        expect(body).not.toBeNull();
        expect(body).toContainElement(screen.getByText("本文"));
    });
});
