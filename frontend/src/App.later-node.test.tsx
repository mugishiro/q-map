...
describe("later node promotion", () => {
    ...
    it("昇格時に既存の後で聞くノードを更新し、新規ノードを作らない", async () => {
        ...
        const rootNode = {
            ...
            updatedAt: now,
            messages: [
                { role: "user" as const, content: "root", createdAt: now },
            ],
        };
        const laterNode = {
            id: "node-later",
        ...
        const listNodesResponses = [
            { items: [rootNode, laterNode] },
            { items: [rootNode, promotedNode] },
        ];
        api.listNodes.mockImplementation(
            async () => listNodesResponses.shift() ?? { items: [] },
        );
        ...
        await waitFor(() => {
            ...
            expect(document.querySelectorAll(".gitk-node.later").length).toBe(
                0,
            ); // laterノードはツリーに表示しない
        });
    });
    ...
    it("下書きを自動入力し、昇格時に同じノードを更新する", async () => {
        ...
        const laterItem = await screen.findByRole("button", {
            name: /元のタイトル/,
        });
        await userEvent.click(laterItem);
        ...
        await waitFor(() => {
            ...
            const userBubble = document.querySelector(
                ".bubble-flat.user .bubble-content",
            );
            const assistantBubble = document.querySelector(
                ".bubble-flat.assistant .bubble-content",
            );
...
describe("new chat title", () => {
    ...
    it("最初の質問からトピック名を生成する", async () => {
        ...
        api.createTopic.mockResolvedValue({
            id: "topic-1",
            name: "これはテストです",
            createdAt: now,
            updatedAt: now,
        ...
        await waitFor(() =>
            expect(api.createTopic).toHaveBeenCalledWith("これはテストです"),
        );
    });
});
