...
const normalizeNode = (n: any): Node => {
    ...
    const type =
        n.type ||
        (Array.isArray(n.messages) && n.messages.length > 0 ? "chat" : "later");
    return { ...n, id, type };
};
    ...
    if (!res.ok) {
        ...
        if (text) {
            try {
                ...
                message =
                    data?.error?.message || data?.message || text || message;
            } catch {
...
export const api = {
    ...
    async updateTopic(topicId: string, name: string): Promise<Topic> {
        const res = await fetch(
            `${API_BASE}/v1/topics/${encodeURIComponent(topicId)}`,
            {
                method: "PATCH",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ name }),
    ...
    async getNodePath(nodeId: string): Promise<PathResponse> {
        const res = await fetch(
            `${API_BASE}/v1/nodes/${encodeURIComponent(nodeId)}/path`,
        );
    ...
    async deleteNode(nodeId: string): Promise<void> {
        const res = await fetch(
            `${API_BASE}/v1/nodes/${encodeURIComponent(nodeId)}`,
            {
                method: "DELETE",
            },
        );
