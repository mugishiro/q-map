...
フロントエンドの React SPA から LLM 呼び出しまでの構成を示す。ホスティングは Vercel。`/api/*` は Vercel の rewrite で API Gateway 経由の Lambda (BFF) にルーティングする。Lambda は DynamoDB/KMS/LLM 各種 API を扱う。
...
[Vercel Hosting]
   │
   │  /api/* だけ API Gateway にルーティング（Vercel rewrite）
   ▼
...
## コンポーネント役割
- **Vercel Hosting**: React SPA をビルド/デプロイ。`/api/*` は rewrite で API Gateway にフォワード。
...
- **環境変数/シークレット**: TABLE 名や KMS ARN などは環境変数。LLM API キーは DB に暗号化保存し、実行時に KMS/Secrets Manager から復号。ホスティング側には平文キーを保持しない。
- **DynamoDB**: `doc/DYNAMODB_DESIGN.md` に詳細。3 テーブル（UserSettings/Topics/Nodes）+ GSI。
- **LLM API**: ユーザー設定で指定されたプロバイダ・モデルにリクエスト。コストはユーザー負担。

## リクエストフロー例（/api/chat）
1. ブラウザ → Vercel → API Gateway（/api/chat）。
...
- **コスト最適化**: Vercel + DynamoDB + Lambda + CloudWatch で月 200〜650 円目安（LLM コストはユーザー負担）。
