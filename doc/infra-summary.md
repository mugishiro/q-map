...
- デプロイ対象: Amplify（GitHub `https://github.com/mugishiro/q-map.git`）、API Gateway (HTTP API)、DynamoDB、KMS、Lambda
...
2. 変更があれば tfvars を修正（例: allowed_origins を実ドメインに差し替え）
3. 実行例（dev）  
   ```bash
...
## フロント連携メモ
- 必要な環境変数（例）: `VITE_API_BASE_URL`（基本 `/api`）、`VITE_API_PROXY_TARGET`（ローカル開発で APIGW にプロキシする場合）
...
## 次にやること
- 実ドメイン確定後、tfvars の allowed_origins を更新して dev/prod で再 apply
...
- dev/prod で Terraform apply 済み（allowed_origins を Amplify ドメインへ更新、Lambda 500 の原因だった aws-sdk を同梱）
