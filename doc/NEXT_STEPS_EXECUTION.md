...
## Vercel Hosting / ルーティング
- 現状: Vercel でホスティング。`vercel.json` で `/api/<*>` → APIGW (`https://{api}/{stage}/<*>`) の rewrite を設定済み。Vercel 環境では `VITE_API_BASE_URL=/api` を渡し、ローカル開発は `.env.dev` で APIGW 直叩き（例: `https://csdq8anoqk.../dev`）。  
- カスタムドメインを入れる場合:  
  - Vercel に独自ドメイン + HTTPS 証明書を設定。  
  - `/api/<*>` → `https://{api-domain}/{stage}/<*>` の rewrite を維持し、`VITE_API_BASE_URL=/api` とする。  
  - SPA fallback は Vercel のデフォルトで `index.html` に解決される。
- 環境変数（Vercel 環境に設定、秘匿情報は置かない）:
  - `VITE_API_BASE_URL`（Vercel は `/api`, ローカルは APIGW 直）、`VITE_APP_STAGE`
...
- シークレット:
  ...
  - プラットフォーム側の固定シークレット（例: OpenRouter プロキシ鍵）を持つ場合は Secrets Manager に保存し、Lambda に特定のシークレット ARN だけ参照権限を付与。ホスティング側の環境変数には秘匿情報を置かない。

## CI/CD 方針
- フロント（Vercel Hosting 接続）:
  - main ブランチ: Vercel のビルド & デプロイ（`npm ci && npm run build`）。環境変数は Vercel の環境ごとに設定済み。
  - PR/ブランチ: Vercel Preview を使うか、別環境を作成するか方針決定。
- バックエンド（Terraform + GitHub Actions 想定）:
  ...
  - デプロイ成果物（API エンドポイント/テーブル名/KMS ARN など）は `terraform output -json` を GitHub Actions の outputs に載せるか、SSM Parameter Store に書き出し、Vercel 側で参照。
...
- Cognito:
  ...
  - User Pool Client: public client, callback/logout に Vercel ドメイン（`/` と `/callback`/`/logout`）と `http://localhost:5173/` を登録。Scope: `openid email profile`。Access Token 1h, Refresh 30d 目安（現状の Terraform 設定）。
- API Gateway HTTP API:
  - JWT オーソライザーで issuer=各 UserPool, audience=ClientId。全ルートに適用。
  - CORS は tfvars の `allowed_origins` で管理（現在は Vercel ドメインと localhost）。
