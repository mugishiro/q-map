...
const DEFAULT_USER_ID = process.env.DEFAULT_USER_ID ?? "user#public";
...
const error = (
    ...
    message: string,
    status = 400,
): APIGatewayProxyResultV2 => json(status, { error: { code, message } });
...
const getUserId = (_event: APIGatewayProxyEventV2): string => DEFAULT_USER_ID;
...
const countChildren = async (
    ...
    const res = await ddb
        ...
        .promise();
    const items = (res.Items as NodeItem[]) ?? [];
    return items.filter((n) => n.userId === userId && n.topicId === topicId)
        .length;
};
...
const deleteTopicWithNodes = async (userId: string, topicId: string) => {
    // Delete Topic
    await ddb
        .delete({ TableName: TOPICS_TABLE, Key: { userId, topicId } })
        .promise();
...
const ALLOWED_LLM_PROVIDERS: LLMProvider[] = [
    "openai",
    "openrouter",
    "anthropic",
    "gemini",
...
const saveUserSettings = async (
    userId: string,
    input: { llmProvider: LLMProvider; model: string; apiKey: string },
) => {
    if (!ALLOWED_LLM_PROVIDERS.includes(input.llmProvider)) {
        throw new AppError("INVALID_PROVIDER", "Unsupported LLM provider", 400);
    }
    if (!input.model)
        throw new AppError("INVALID_MODEL", "model is required", 400);
    if (!input.apiKey)
        throw new AppError("LLM_API_KEY_MISSING", "apiKey is required", 400);
...
const ensureBody = <T extends Record<string, unknown>>(
    body: T,
    keys: (keyof T)[],
): boolean => {
...
const attachParentId = <T extends { parentId?: string }>(
    item: T,
    parentId?: string | null,
): T => {
    const next = { ...item };
    ...
    try {
        ...
        if (method === "GET" && path === "/v1/me/settings") {
            ...
            return json(200, {
                ...
                apiKeyMasked: settings?.apiKeyEncrypted
                    ? maskApiKey(await decryptApiKey(settings.apiKeyEncrypted))
                    : null,
            });
        }
        ...
        if (method === "POST" && path === "/v1/me/settings") {
            ...
            if (!ensureBody(body, ["llmProvider", "model", "apiKey"])) {
                return error(
                    "VALIDATION_ERROR",
                    "llmProvider, model, apiKey are required",
                );
            }
        ...
        if (method === "POST" && mSummary) {
            ...
            const summaries = items
                .map((n) => `- ${n.label || n.nodeId}: ${n.summary || n.title}`)
                .join("\n");
            return json(200, {
                topicId: mSummary.topicId,
                summary: summaries || "データがありません。",
            });
        }
        ...
        if (method === "POST" && path === "/v1/nodes") {
            ...
            if (!ensureBody(body, ["topicId", "summary"]))
                return error(
                    "VALIDATION_ERROR",
                    "topicId and summary are required",
                );
            ...
            if (body.parentId) {
                ...
                if (parent.topicId !== body.topicId) {
                    return error(
                        ...
                        "Parent node does not belong to the topic",
                        400,
                    );
                }
                if (parent.type === "later") {
                    return error(
                        "PARENT_NOT_ALLOWED",
                        "Cannot add later node under another later node",
                        400,
                    );
                }
            }
            ...
            const label =
                body.label ||
                (await generateLabel(body.topicId!, parentId, userId));
        ...
        if (method === "GET" && mNodePath) {
            ...
            const normalizedPath = pathArr.map((n) =>
                normalizeNodeType(n, true),
            );
        ...
        if (method === "PATCH" && mNode) {
            ...
            await ddb
                .update({
                    ...
                    UpdateExpression:
                        "SET title = :title, summary = :summary, updatedAt = :updatedAt",
        ...
        if (method === "DELETE" && mNode) {
            ...
            if (node.type !== "later") {
                return error(
                    "DELETE_NOT_ALLOWED",
                    "Only later nodes can be deleted",
                    400,
                );
            }
            await ddb
        ...
        if (method === "POST" && path === "/v1/chat") {
            ...
            const targetNode = body.nodeId
                ? await findNodeById(userId, body.nodeId)
                : null;
            if (body.nodeId && !targetNode)
                return error("NODE_NOT_FOUND", "target node not found", 404);
            if (targetNode && targetNode.topicId !== body.topicId) {
                return error(
                    "NODE_TOPIC_MISMATCH",
                    "target node does not belong to the topic",
                    400,
                );
            }
            ...
            if (body.baseNodeId) {
                ...
                if (baseNode.topicId !== body.topicId) {
                    return error(
                        "BASE_NODE_TOPIC_MISMATCH",
                        "baseNode does not belong to the topic",
                        400,
                    );
                }
            } else if (targetNode?.parentId) {
                baseNode = await getNode(
                    targetNode.topicId,
                    targetNode.parentId,
                );
            ...
            if (targetNode) {
                if (targetNode.type && targetNode.type !== "later") {
                    return error(
                        "NODE_NOT_LATER",
                        "target node is not a later node",
                        400,
                    );
                }
                await ddb
